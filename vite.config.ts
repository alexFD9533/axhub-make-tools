...
function shouldIgnoreDevServerWatch(filePath: string): boolean {
  ...
  return normalized.includes('/.axhub/make/')
    || normalized.includes('/canvas-assets/')
...
export default defineConfig(({ command }) => {
  ...
  const config: any = {
    ...
    server: {
      ...
      hmr: { overlay: false },
      watch: {
        ignored: shouldIgnoreDevServerWatch,
      },
      headers: {
    ...
    build: {
      ...
      assetsInlineLimit: 6 * 1024 * 1024, // 6MB – 内联卫星地图和背景图片，避免发布后缓存/路径问题
      rollupOptions: {
        input: rollupInput,
