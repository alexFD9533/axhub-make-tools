...
export function parseHashPage(hash: string): string | null {
    ...
    const slashRoutePageId = rawHash.startsWith('/') ? rawHash.slice(1).split(/[?#]/u)[0] : '';
    if (slashRoutePageId) {
        const normalizedSlashRoute = normalizePageId(slashRoutePageId);
        if (normalizedSlashRoute) return normalizedSlashRoute;
    }
